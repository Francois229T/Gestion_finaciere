...
function generernoteservice(
    ...
    try {
        ...
        $options->set('defaultFont', 'Verdana'); // Police par défaut Verdana
        ...
        <head>
            ...
            <style>
                ...
                .header-right-block .activity-name-subtitle {
                    font-size: 11pt; /* Garde 11pt pour le PDF, cohérent avec Excel */
                    font-weight: bold;
                    margin-top: 2px;
                    text-transform: uppercase;
                    word-wrap: break-word;
                ...
                .participants-table td {
                    font-weight: bold; /* Les données du tableau sont en gras */
                }
                ...
                .signature-section td {
                    ...
                    padding: 0;
                    border: none;
                    text-align: center; /* Maintenu centré pour être cohérent avec l'Excel modifié */
                }
                .signature-section p {
                    margin: 2px 0;
                    line-height: 1.4;
    ...
    try {
        ...
        // AJOUTÉE : Pour que le document Excel soit en mode paysage, comme le PDF.
        $sheet->getPageSetup()->setOrientation(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_LANDSCAPE);


        // Styles par défaut : Utilisation de Verdana pour être cohérent avec le PDF
        $spreadsheet->getDefaultStyle()->getFont()->setName('Verdana')->setSize(11);
        ...
        $sheet->getStyle('D1')->getFont()->setSize(9.5)->setBold(true); // Taille 9.5pt pour être cohérent avec le PDF
        ...
        $sheet->getStyle('A' . $row_excel)->getFont()->setBold(true)->setSize(11); // Taille 11pt pour être cohérent avec le PDF
        $sheet->getStyle('A' . $row_excel)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A' . $row_excel . ':E' . $row_excel);
        $row_excel+=2; // Two empty lines after the main title

        // Détails de l'activité (Période et Lieu)
        ...
        // Bloc de signature centré pour Excel (cohérent avec le PDF)
        $sheet->setCellValue('A' . $signature_start_row, 'LE CMAP');
        $sheet->getStyle('A' . $signature_start_row)->getFont()->setBold(true);
        $sheet->getStyle('A' . $signature_start_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A' . $signature_start_row . ':E' . $signature_start_row);

        $sheet->setCellValue('A' . ($signature_start_row + 1), '(Signature)');
        $sheet->getStyle('A' . ($signature_start_row + 1))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A' . ($signature_start_row + 1) . ':E' . ($signature_start_row + 1));

        $sheet->setCellValue('A' . ($signature_start_row + 4), htmlspecialchars(mb_strtoupper($activity_data['responsable_titre'] ?? 'Nom et Titre Responsable', 'UTF-8')));
        $sheet->getStyle('A' . ($signature_start_row + 4))->getFont()->setBold(true)->setUnderline(true);
        $sheet->getStyle('A' . ($signature_start_row + 4))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A' . ($signature_start_row + 4) . ':E' . ($signature_start_row + 4));
