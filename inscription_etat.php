...
<head>
    ...
    <title>Inscription Agent de l'État - Plateforme Financière</title>
...
<body>
            ...
            <ul>
                ...
                <li class="dropdown">
                    ...
                    <div class="dropdown-content">
                        ...
                        <a href="inscription_etat.php" class="active">Inscription Agent de l'État</a>
                        <a href="inscription_morale.php">Inscription Personne Morale</a>
                        <a href="gerer-participants.html">Gérer les participants</a>
                        <a href="creer_activite.php">Créer une activité</a>
                        <a href="gerer-activite.html">Gérer les activités</a>
                    </div>
                </li>
                <li class="dropdown">
    ...
    <main>
        <section class="form-section">
            <h2>Inscription d'un Agent de l'État</h2>
            <p class="form-description">Remplissez ce formulaire pour l'enrôlement d'un agent de l'État.</p>

            <form action="#" method="post" enctype="multipart/form-data">
                <fieldset>
                    <legend>Informations d'Identification de l'Agent</legend>
                    <div class="form-group">
                        <label for="matricule-agent">Matricule :</label>
                        <input type="text" id="matricule-agent" name="matricule_agent" required placeholder="Matricule de l'agent de l'État">
                    </div>
                    <div class="form-group">
                        <label for="ifu-agent">IFU (Identifiant Fiscal Unique) :</label>
                        <input type="text" id="ifu-agent" name="ifu_agent" placeholder="IFU de l'agent">
                    </div>
                    <div class="form-group">
                        <label for="nom-agent">Nom :</label>
                        <input type="text" id="nom-agent" name="nom_agent" required>
                    </div>
                    <div class="form-group">
                        <label for="prenom-agent">Prénom(s) :</label>
                        <input type="text" id="prenom-agent" name="prenom_agent" required>
                    </div>
                    <div class="form-group">
                        <label for="date-naissance-agent">Date de naissance :</label>
                        <input type="date" id="date-naissance-agent" name="date_naissance_agent" required>
                    </div>
                    <div class="form-group">
                        <label for="lieu-naissance-agent">Lieu de naissance :</label>
                        <input type="text" id="lieu-naissance-agent" name="lieu_naissance_agent" required>
                    </div>
                    <div class="form-group">
                        <label for="diplome-agent">Diplôme le plus élevé :</label>
                        <input type="text" id="diplome-agent" name="diplome_agent" placeholder="Ex: Master en Administration Publique">
                    </div>
                </fieldset>

                <fieldset>
                    <legend>Coordonnées Bancaires de l'Agent</legend>
                    <div id="bank-accounts-container-agent">
                        <div class="bank-account-block" style="border: 1px dashed #e0e0e0; padding: 15px; margin-bottom: 15px; border-radius: 5px;">
                            <div class="form-group">
                                <label for="bank-agent-1">Banque:</label>
                                <input type="text" id="bank-agent-1" name="bank_agent[]" required placeholder="Nom de la banque">
                            </div>
                            <div class="form-group">
                                <label for="account-number-agent-1">Numéro de compte bancaire:</label>
                                <input type="text" id="account-number-agent-1" name="account_number_agent[]" required placeholder="RIB/Numéro de compte">
                            </div>
                            <div class="form-group">
                                <label for="rib-pdf-agent-1">Copie PDF du RIB:</label>
                                <input type="file" id="rib-pdf-agent-1" name="rib_pdf_agent[]" accept="application/pdf" required>
                                <small>Fichier PDF uniquement.</small>
                            </div>
                            <button type="button" class="btn secondary btn-small remove-bank-account-agent" style="display:none;">Supprimer ce compte</button>
                        </div>
                    </div>
                    <button type="button" class="btn primary btn-small" id="add-bank-account-agent" style="margin-top: 10px;">Ajouter un autre compte bancaire</button>
                </fieldset>

                <div class="form-actions">
                    <button type="submit" class="btn primary">Enregistrer l'Agent</button>
    ...
    <script>
        // JavaScript pour ajouter/supprimer des blocs de comptes bancaires pour l'agent
        document.addEventListener('DOMContentLoaded', function() {
            const container = document.getElementById('bank-accounts-container-agent');
            const addButton = document.getElementById('add-bank-account-agent');
            let accountIndex = 1;

            function updateRemoveButtons() {
                const removeButtons = container.querySelectorAll('.remove-bank-account-agent');
            ...
            addButton.addEventListener('click', function() {
                ...
                newBlock.innerHTML = `
                    <div class="form-group">
                        <label for="bank-agent-${accountIndex}">Banque:</label>
                        <input type="text" id="bank-agent-${accountIndex}" name="bank_agent[]" required placeholder="Nom de la banque">
                    </div>
                    <div class="form-group">
                        <label for="account-number-agent-${accountIndex}">Numéro de compte bancaire:</label>
                        <input type="text" id="account-number-agent-${accountIndex}" name="account_number_agent[]" required placeholder="RIB/Numéro de compte">
                    </div>
                    <div class="form-group">
                        <label for="rib-pdf-agent-${accountIndex}">Copie PDF du RIB:</label>
                        <input type="file" id="rib-pdf-agent-${accountIndex}" name="rib_pdf_agent[]" accept="application/pdf" required>
                        <small>Fichier PDF uniquement.</small>
                    </div>
                    <button type="button" class="btn secondary btn-small remove-bank-account-agent">Supprimer ce compte</button>
                `;
            ...
            container.addEventListener('click', function(event) {
                if (event.target.classList.contains('remove-bank-account-agent')) {
